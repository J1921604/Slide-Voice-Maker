...
async def generate_voice(text, output_path, voice="ja-JP-NanamiNeural"):
    ...
    if cm is None:
        ...
        try:
            import edge_tts
            try:
                communicate = edge_tts.Communicate(text, voice, proxy=proxy) if proxy else edge_tts.Communicate(text, voice)
            except TypeError:
...
async def generate_single_audio(
    ...
    if script and script.strip():
        ...
        await generate_voice(text_to_speak, audio_path, voice=_resolve_voice_preset(voice))
        return audio_path
    return ""
